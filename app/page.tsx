...
// Safe hydration-friendly client device check
  ...
  return (
    <div className="min-h-screen bg-[#f9f8f6] text-stone-800 w-full overflow-x-hidden antialiased selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12 space-y-8 sm:space-y-16">
        ...
        {/* Dynamic Mobile/Desktop Metric Grid - Clean vertical stacking on mobile, grids on PC */}
        ...
        {/* Marketplace Plate */}
        <div id="catalog-section" className="bg-white border border-stone-200/40 rounded-2xl sm:rounded-3xl p-4 sm:p-8 space-y-6 shadow-xs">
          ...
          {loading ? (
            ...
            <div className="w-full">
              {/* FIXED GRID SYSTEM: 
                  - Mobile (`grid-cols-2` + `gap-3`): Generates a highly authentic, comfortable, and spacious 2x2 grid without horizontal squishing.
                  - Tablet (`md:grid-cols-3`): Scales gracefully.
                  - PC / Large Screens (`lg:grid-cols-4 xl:grid-cols-5` + `gap-6`): Preserves original gorgeous layout completely intact.
              */}
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-6 pt-2 pb-3 w-full">
        ...
        <div className="bg-[#edf1e8]/60 rounded-2xl sm:rounded-3xl p-6 sm:p-10 border border-emerald-200/20 space-y-6 sm:space-y-8">
          ...
          {/* Stacks beautifully as 1 column on mobile, transitions smoothly to 3 columns on desktop */}
        ...
        {/* Supplier Directory - Fixed to render perfectly on both Mobile and PC */}
